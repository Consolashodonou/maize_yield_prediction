...
def load_saved_model_if_exists():
    if os.path.exists("catboost_yield_model.pkl"):
        with open("catboost_yield_model.pkl", "rb") as f:
            saved = pickle.load(f)
        return saved["model"], saved["train_columns"]
    return None, None
...
try:
    ...
    saved_model, saved_columns = load_saved_model_if_exists()

    if saved_model is not None:
        model = saved_model
        train_columns = saved_columns

        # Recompute metrics on current dataset for UI consistency
        _, _, X_test, y_test, y_pred_test, metrics = train_catboost(train_bytes)

        st.success(
            "✅ Pre-trained model loaded from pickle file.\n\n"
            f"Performance on current dataset — "
            f"R² = **{metrics['R²']:.4f}** | "
            f"RMSE = **{metrics['RMSE']:.1f} kg/ha**"
        )
    ...
    st.error("⚠️ **Training file issue.**")
    st.code(str(e))
    st.stop()
